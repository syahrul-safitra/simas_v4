    ...
    <div class="row g-4">
        <div class="col-sm-12">
            <div class="bg-light rounded h-100 p-4">
                ...
                <form action="{{ url('dashboard/informasi/' . $informasi->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="deskripsi" class="form-label">Deskripsi</label>
                        <input type="text" class="form-control @error('deskripsi') is-invalid @enderror" name="deskripsi"
                            id="deskripsi" value="{{ @old('deskripsi', $informasi->deskripsi) }}" autocomplete="off"
                            autofocus>
                        @error('deskripsi')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('tujuan') is-invalid @enderror" name="tujuan"
                            id="tujuan" value="{{ @old('tujuan', $informasi->tujuan) }}" autocomplete="off">
                        @error('tujuan')
                            <div class="invalid-feedback text-red">{{ $message }}</div>
                        @enderror
                    </div>
